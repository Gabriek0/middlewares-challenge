...
function checksCreateTodosUserAvailability(request, response, next) {
  ...
  if (user.todos.length === 10 && !user.pro) {
    return response.status(403).json({ error: "Limit of todos is exceeded." });
  }
...
function checksTodoExists(request, response, next) {
  ...
  const isUuid = uuidTester.test(id);
  if (!isUuid) return response.status(400).json({ error: "Id is not a UUID." });
